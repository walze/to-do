...
export const addTodo: QueryResolvers['addTodo'] = (_, {data}) => useConnection(
    async (conn) => {
      assert(data, 'no data provided');
      assert(data.content, 'no content provided');
      assert(data.user, 'no user provided');

      const user = await User.findOne({where: {name: data.user.name}}) ||
        await addUser({name: data.user.name});
